<div {{ $attributes->class(['']) }} x-data="{
    ...
    <label for="{{ $wireId }}">
        ...
        <div class="focus-within:ring-primary focus-within:border-primary @error($wireId) border-red-500 @enderror mt-1 flex w-full max-w-[200px] overflow-hidden rounded-md border border-gray-300 focus-within:ring-1 sm:text-sm">
            ...
            <input x-model.value="value" wire:loading.class="opacity-20" wire:loading.attr="readonly" wire:target="{{ $wireTarget }}" type="text" name="{{ $wireId }}" id="{{ $wireId }}" class="{{ $inputClass }} w-full border-none px-3 py-1.5 text-center focus:outline-none focus:ring-0" tabindex="-1" readonly />
